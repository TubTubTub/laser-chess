...
class LaserDraw:
    ...
    def add_laser(self, laser_result, laser_colour):
        ...
        laser_rotation = [laser_result.laser_path[0][1]]
        
        for i in range(1, len(laser_result.laser_path)):
            previous_direction = laser_result.laser_path[i-1][1]
            current_direction = laser_result.laser_path[i][1]
        ...
        if laser_result.hit_square_bitboard != EMPTY_BB:
            ...
            laser_result.laser_path[-1] = (laser_result.laser_path[-1][0], laser_result.laser_path[-2][1].get_opposite())
            laser_rotation[-1] = laser_result.laser_path[-2][1].get_opposite()

        laser_path = [(coords, rotation, type) for (coords, dir), rotation, type in zip(laser_result.laser_path, laser_rotation, laser_types)]
